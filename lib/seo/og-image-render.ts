...
function wrapCenteredLines(text: string, maxChars: number, maxLines: number): string[] {
...
export async function renderPhotoHeroOgPng(input: {
  ...
  const titleLines = wrapCenteredLines(input.title, 14, 2).map(escapeXml);
  ...
  const line2 = escapeXml(input.line2.trim() || "");
  const badge = escapeXml(input.badge.trim() || "유아독존");
  const bar = escapeXml(input.bar.trim() || "");
