...
export function NearbyRegionalLinks({ currentLabel, intro, nearby }: Props) {
  ...
  const title = sample
    ? getRegionalServiceConfig(resolvePageCategory(sample)).title
    : "애견미용학원";

  const text =
    intro ??
    `${currentLabel}에서 ${title}을 알아보는 분들이 근방에서 함께 검색·방문하는 지역입니다.`;
  ...
  return (
    <section className="mb-12 rounded-2xl border border-gray-100 bg-gradient-to-br from-slate-50 to-white p-6 shadow-[var(--card-shadow)] sm:p-8">
      ...
      <h2 className="mt-1 text-lg font-bold text-foreground sm:text-xl">
        {currentLabel} 인근에서 많이 함께 찾는 {title} 지역
      </h2>
      <p className="mt-3 text-sm leading-relaxed text-muted">{text}</p>

      <ul className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {nearby.map((page) => {
          const config = getRegionalServiceConfig(resolvePageCategory(page));
          return (
          <li key={`${resolvePageCategory(page)}-${page.slug}`}>
            <Link
